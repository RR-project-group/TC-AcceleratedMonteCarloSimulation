...
DATA_DIR = os.path.join(BASE_DIR, 'BSresult')  # Automatically adapts the path, no manual changes needed
...
for metric in metrics:
    ...
    plt.title(f'BS - {metric} Comparison Across Settings')
    plt.tight_layout()
    plt.savefig(os.path.join(DATA_DIR, f'BS_{metric}_boxplot.png'))
    plt.close()
...
plt.title('BS - Average Error Metrics by Setting')
